...
def setup():
    ...
    parser.add_argument("--skip_des_y3", action="store_true")
...
def main():
    ...
    with open(os.path.join(pred_dir, "configs.yaml"), "w") as f:
        yaml.dump_all([mlp_conf, dlss_conf, msfm_conf], f)
    ...
    model.setup_grid_loss_step(
        ...
        mutual_info_estimator=mlp_conf.get("mutual_info_estimator", "variational"),
        clip_by_global_norm=mlp_conf.get("clip_by_global_norm", 1.0),
        mutual_info_kwargs={"full_covariance": mlp_conf.get("full_covariance", False)},
    )
    ...
    vali_steps, vali_losses_history = [], []
    ...
    for i, (cl_batch, cosmo_batch) in tqdm(enumerate(cl_dset_train), total=n_steps + 1):
        ...
        if i > 0 and i % vali_every == 0:
            ...
            tb_writer.flush()
            tqdm.write(f"step {i:>7d}  vali_loss = {vali_loss:.4f}")
    ...
    cls_test = np.array(out_dict["grid/cls/test"], dtype=np.float32)
    grid_cosmos = np.array(out_dict["grid/cosmos/test"], dtype=np.float32)
    ...
    n_grid_obs = mlp_conf.get("n_grid_obs", 4)
...
def _save_loss_curve(pred_dir, pred_file, train_steps, train_losses, vali_steps, vali_losses, log_every):
    ...
    # skip first few log points to avoid the noisy initial plateau
